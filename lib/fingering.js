...
/**
 * Аппликатура по реальной высоте звука (октава), а не только по имени ноты.
 */
export function recorderMidiForSung(sungMidi, recorderType = 'soprano_c') {
  const midi = Math.round(sungMidi);
  const pc = pitchClassFromMidi(midi);

  const candidates = Object.keys(FINGERING).map(Number)
    .filter((m) => pitchClassFromMidi(m) === pc && FINGERING[m])
    .sort((a, b) => a - b);
...
export function getFingering(midi, recorderType = 'soprano_c') {
  const lookupMidi = recorderType === 'alto_f' ? midi + 7 : midi;
  const holes = FINGERING[lookupMidi];
  if (!holes) {
    return null;
  }
  return {
