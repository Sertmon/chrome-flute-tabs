...
export class PitchVisual {
  ...
  constructor(canvas, opts = {}) {
    ...
    this.#fullMinMidi = this.#minMidi;
    this.#fullMaxMidi = this.#maxMidi;
    this.#viewMinMidi = this.#minMidi;
    this.#viewMaxMidi = this.#maxMidi;
  ...
  setOnPitchZoomChange(callback) {
    this.#onPitchZoomChange = callback;
  }

  setRange(minMidi, maxMidi) {
    this.#minMidi = minMidi;
    this.#maxMidi = maxMidi;
    this.#fullMinMidi = minMidi;
    this.#fullMaxMidi = maxMidi;
    this.resetPitchView();
  }

  resetPitchView() {
    this.#viewMinMidi = this.#fullMinMidi;
    this.#viewMaxMidi = this.#fullMaxMidi;
    this.#notifyPitchZoomChange();
    this.draw();
  }

  isPitchZoomed() {
    return this.#viewMaxMidi - this.#viewMinMidi < this.#fullMaxMidi - this.#fullMinMidi - 0.5;
  }

  canPanPitch() {
    return this.isPitchZoomed();
  }

  zoomIn() {
    const center = this.#focusCenterMidi();
    const span = this.#viewMaxMidi - this.#viewMinMidi;
    const newSpan = Math.max(MIN_VIEW_SEMITONES, span * ZOOM_STEP);
    this.#viewMinMidi = center - newSpan / 2;
    this.#viewMaxMidi = center + newSpan / 2;
    this.#clampPitchView();
    this.#notifyPitchZoomChange();
    this.draw();
  }
  ...
  zoomOut() {
    const center = this.#focusCenterMidi();
    const span = this.#viewMaxMidi - this.#viewMinMidi;
    const fullSpan = this.#fullMaxMidi - this.#fullMinMidi;
    const newSpan = Math.min(fullSpan, span / ZOOM_STEP);
  ...
  panPitchSemitones(delta) {
    if (!this.canPanPitch()) {
      return;
    }
    this.#viewMinMidi += delta;
    this.#viewMaxMidi += delta;
    this.#clampPitchView();
    this.#notifyPitchZoomChange();
    this.draw();
  }
  ...
  #focusCenterMidi() {
    if (this.#current && Number.isFinite(this.#current.midiFloat)) {
      return this.#current.midiFloat;
    }
    return (this.#viewMinMidi + this.#viewMaxMidi) / 2;
  }

  #clampPitchView() {
    let span = this.#viewMaxMidi - this.#viewMinMidi;
    if (span < MIN_VIEW_SEMITONES) {
      const center = (this.#viewMinMidi + this.#viewMaxMidi) / 2;
      span = MIN_VIEW_SEMITONES;
      this.#viewMinMidi = center - span / 2;
      this.#viewMaxMidi = center + span / 2;
    }

    const fullSpan = this.#fullMaxMidi - this.#fullMinMidi;
    if (this.#viewMaxMidi - this.#viewMinMidi >= fullSpan - 0.5) {
      this.#viewMinMidi = this.#fullMinMidi;
      this.#viewMaxMidi = this.#fullMaxMidi;
      return;
    }

    if (this.#viewMinMidi < this.#fullMinMidi) {
      const shift = this.#fullMinMidi - this.#viewMinMidi;
      this.#viewMinMidi += shift;
      this.#viewMaxMidi += shift;
    }
    if (this.#viewMaxMidi > this.#fullMaxMidi) {
      const shift = this.#viewMaxMidi - this.#fullMaxMidi;
      this.#viewMinMidi -= shift;
      this.#viewMaxMidi -= shift;
    }

    this.#viewMinMidi = Math.max(this.#fullMinMidi, this.#viewMinMidi);
    this.#viewMaxMidi = Math.min(this.#fullMaxMidi, this.#viewMaxMidi);
  }

  #notifyPitchZoomChange() {
    this.#onPitchZoomChange?.({
      zoomed: this.isPitchZoomed(),
      canPan: this.canPanPitch(),
    });
  }
  ...
  #midiToX(midiFloat, width, pad, axisW) {
    ...
    const ratio = (midiFloat - this.#viewMinMidi) / span;
    const left = pad + axisW;
    return left + ratio * (width - left - pad);
  }

  #viewKeyWidth(width, pad, axisW) {
    const span = this.#viewMaxMidi - this.#viewMinMidi;
    const plotW = width - pad - axisW - pad;
    return span > 0 ? plotW / span : plotW;
  }

  #viewMidiMinInt() {
    return Math.max(this.#fullMinMidi, Math.floor(this.#viewMinMidi));
  }

  #viewMidiMaxInt() {
    return Math.min(this.#fullMaxMidi, Math.ceil(this.#viewMaxMidi));
  }
  ...
  #drawPiano(ctx, width, top, height, pad, axisW) {
    const keyW = this.#viewKeyWidth(width, pad, axisW);
    const whiteH = height - 14;
    const midiMin = this.#viewMidiMinInt();
    const midiMax = this.#viewMidiMaxInt();
    ...
    for (let midi = midiMin; midi <= midiMax; midi += 1) {
      ...
      this.#drawKeyLabel(ctx, midi, x + keyW / 2, top, whiteH, keyW);
    }
  ...
  #drawKeyLabel(ctx, midi, cx, top, whiteH, keyW) {
    const pc = ((midi % 12) + 12) % 12;
    const letter = NOTE_LETTERS[pc];
    const octave = Math.floor(midi / 12) - 1;

    if (keyW >= 12 && !letter.includes('#')) {
      ctx.fillStyle = COLORS.label;
      ctx.font = keyW >= 18 ? 'bold 10px system-ui, sans-serif' : 'bold 9px system-ui, sans-serif';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(letter, cx, top + whiteH * 0.45);
    }
    ...
    if (midi % 12 === 0 && keyW >= 8) {
      ...
      ctx.fillText(`C${octave}`, cx, top + whiteH + 10);
    }
  }
  ...
  #drawRoll(ctx, width, top, height, pad, axisW, anchorMs) {
    ...
    for (let midi = this.#viewMidiMinInt(); midi <= this.#viewMidiMaxInt(); midi += 1) {
