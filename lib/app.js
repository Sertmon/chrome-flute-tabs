...
const session = new NoteRecorder(renderSequence);
  ...
  function updateLive(note, freq) {
    ...
    liveNote.textContent = note.labelRu;
    liveNote.classList.remove('out-of-range');
    liveFreq.textContent = `${Math.round(freq)} Hz`;
    liveRu.textContent = note.name;

    const fingering = getFingering(note.recorderMidi);
  ...
  function renderSequence() {
    ...
    notes.forEach((note, index) => {
      ...
      item.innerHTML = `
        ...
        <span class="note-label">${note.labelRu} <small>${note.name}</small></span>
