...
export function initApp(root = document) {
  ...
  let pitchVisual = visualCanvas
    ? new PitchVisual(visualCanvas, initialPiano)
    : null;
  let rafId = 0;
  ...
  async function updateTabTargetHint() {
    ...
    try {
      ...
      tabTargetHint.textContent = `«${title}». «Слушать» → в диалоге Chrome выберите эту вкладку `
        + 'и включите «Поделиться звуком вкладки».';
      tabTargetHint.hidden = false;
    } catch {
      tabTargetHint.hidden = true;
    }
  ...
  async function startListening() {
    ...
    try {
      if (useTab) {
        liveHint.textContent = 'Откроется диалог Chrome — выберите вкладку YouTube '
          + 'и включите «Поделиться звуком вкладки»…';
        mediaStream = await openDisplayMediaAudioStream();
  ...
  function renderSequence() {
    ...
    notes.forEach((note, index) => {
      ...
      const recorderMidi = pitchClassToRecorderMidi(note.pitchClass, note.sungMidi, settings);
      const fingering = getFingering(recorderMidi, settings.recorderType);
...
function describeCaptureError(err, useTab) {
  ...
  if (useTab) {
    ...
    if (name === 'NotAllowedError' && message.includes('getDisplayMedia')) {
      return [
        'Диалог захвата закрыт.',
        'Снова «Слушать» → выберите вкладку YouTube → включите «Поделиться звуком вкладки».',
      ];
    }
    return [
      explained || message,
      '«Слушать» → вкладка YouTube в диалоге → галочка звука вкладки.',
    ];
  }
